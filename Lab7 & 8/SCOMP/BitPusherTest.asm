...
ORG 0
;Start:
	; Brief example of using a subroutine --
	; There is a subroutine further below that delays for some multiple
	; of 0.1 seconds.  It uses the value in AC
	; as the number of 0.1 second increments to delay.
	
	;LOADI  &B0111
	;OUT    LEDs        ; turn on some LEDs.
	
	;LOADI  1
	;CALL   DelayAC     ; Call the subroutine with 1 in AC
	
	; By the way, the last few comments are terrible, right?
	; They don't describe anything about what the code
	; actually *does*.  They're intentionally bad here because
	; we want you to have to decipher what this example does.
	; But the comments throughout the rest of the code are
	; hopefully a lot better, and comments you make on *your* code
	; should be better.
	
	;LOADI  &B1000
	;OUT    LEDs        ; turn on some other LEDs

	;LOADI  1
	;CALL   Negate      ; Call negate subroutine
	;CALL   DelayAC     ; Call the delay subroutine
...
;*******************************************************************************
; L2Estimate:  Pythagorean distance (L2 norm) estimation
